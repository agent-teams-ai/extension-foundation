...
| Portable response correlation | `portable protocol rejects stale, expired, malformed, oversized, and incorrectly correlated frames` | Passed for request and operation IDs, fixed deadline, graph generation, module activation generation and host incarnation; authenticated channel establishment and an operation journal remain unimplemented |
