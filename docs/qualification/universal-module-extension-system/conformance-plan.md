...
| Required, optional and ordered-many cardinality | `qualification bindings preserve required, optional, and ordered-many semantics`; `qualification bindings distinguish a missing optional binding from explicit unbound` | A resolved empty provider list is the disposable compiler's explicit unbound outcome, while a missing coordinate fails closed; no executable fixture yet proves that failure of a selected optional provider aborts a lifecycle candidate, and no product grammar is admitted |
...
| Cycles, self-cycles and independent oracle | `native compiler agrees with Graphlib on generated directed-graph validity`; `ID-DAG chain compilation remains stack-safe and reports non-gating timings` | Passed; Graphlib remains test-only |
...
| 1,000 and 10,000-node ID-DAG chain stack/timing evidence | `ID-DAG chain compilation remains stack-safe and reports non-gating timings` | Stack safety passed and five diagnostic timing samples were recorded per size. No timing or memory threshold is enforced. Wide/dense graphs, peak RSS, retained memory, and production SLOs remain unproved |
...
| Prepare, start, readiness, publication and shutdown ordering | two-built-in rehearsal; readiness/publication cases; shutdown drain/order, reentrant single-flight and reentrant caller-getter terminal-gate cases | Passed only for the synthetic activation/stop projection; it does not prove selected-optional-provider failure semantics or separate drain, retirement and migration plans |
...
| Recovery across every represented durable phase | `crash recovery decisions bind readiness to runtime and module attempts at durable boundaries` | Prepared, started, ready, published, draining and retired checkpoints are serialized and restored into fresh in-memory coordinators before reconciliation; synthetic ready observations bind one exact runtime and module-attempt identity |
| Corrupt, stale, conflicting and unknown recovery evidence | recovery boundary cases with malformed shapes, stale tuples, uncertain outcomes, conflicting generations and missing or mismatched runtime/module-attempt identity | Passed fail-closed as `CONTROLLED_RECOVERY`; no durable or multi-module receipt aggregation is implemented |
| Portable response correlation | `portable protocol rejects stale, expired, malformed, oversized, and miscorrelated frames` | Passed through a host-created opaque dispatch receipt for request/operation IDs, authority scope, extension instance, fixed deadline, graph/module generations, host incarnation, and reversed endpoint direction; authenticated channel establishment and an operation journal remain unimplemented |
