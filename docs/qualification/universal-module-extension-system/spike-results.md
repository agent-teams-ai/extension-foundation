...
| Property and differential graph | Seeded runs preserve replayability: 200 input permutations keep one digest; 500 generated directed graphs agree with Graphlib on cycle validity, while both orders independently satisfy every source edge | Graphlib is not selected as production authority |
| Large ID-DAG chain | Five 1,000-node and five 10,000-node chain samples report non-gating timings; the 10,000-node cycle remains stack-safe and heap delta is diagnostic only | Any timing or memory threshold, wide/dense graph behavior, peak RSS, retained memory, cross-machine calibrated percentiles, and a production SLO gate |
...
| Cordis 4.0.1 | A private Fiber owns resource cleanup; its hooks preserve the coordinator-owned two-generation trace shape, and disposer identity remains distinct for overlapping coordinator incarnations in one authority scope | An independent lifecycle implementation, semantic equivalence, closed-world compile, product readiness, publication authority, recovery or isolation |
...
trivial adapter does not demonstrate a net product benefit; a future real
consumer must use the predeclared multi-factor scorecard.
