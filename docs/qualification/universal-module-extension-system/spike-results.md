...
| Concurrent activation | 100 starts with one operation identity and fingerprint share one activation; callers can cancel their own wait without cancelling shared startup; a changed declared activation-source digest, scope, policy or cleanup input conflicts; distinct operation identities remain separate competing candidates; caller-owned identity is snapshotted before admission; expected-active CAS permits one publisher | Resolver-to-hook digest binding, multi-process consensus or distributed admission |
