...
| Readiness and rollback | Complete descriptor-based hook and readiness preflight runs before module effects; only plain/null-prototype records are admitted; failures carry module/phase attribution; explicit probe/inert policy blocks dependents; failed candidate leaves active routing unchanged | Host isolation for untrusted hook objects, product-specific health policy and durable recovery store |
