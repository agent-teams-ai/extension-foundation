...
| Concurrent activation | 100 same-fingerprint starts share one activation; callers can cancel their own wait without cancelling shared startup; a changed declared activation-source digest, scope, policy or cleanup input conflicts; caller-owned identity is snapshotted before admission; expected-active CAS permits one publisher among different candidates | Resolver-to-hook digest binding, multi-process consensus or distributed admission |
| Readiness and rollback | Complete descriptor-based hook and readiness preflight runs before module effects; explicit probe/inert policy blocks dependents; failed candidate leaves active routing unchanged | Host isolation for untrusted hook objects, product-specific health policy and durable recovery store |
