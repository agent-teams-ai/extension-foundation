...
compared Extension Foundation `e1a3492` with Agent Runtime `493c6c3`. Their
...
| Product | Frontend was promoted to Static V1 GO from self-asserted file records | Downgraded it to a no-go measurement candidate and added exact Git/AST/wiring verification |
| Product | Agent Runtime was recorded before its Codex and Claude Code production paths landed | Refreshed exact evidence to `493c6c3`; admitted `L0` Pure DI source architecture while keeping runtime behavior and `L1-L5` closed |
| Product | Codex and Claude Code could be mistaken for interchangeable providers | Recorded them as sibling capabilities with different contracts and one host-owned lifetime |
| Testing | Claude Code lacks the direct default-root test shape already present for Codex | Recorded a non-blocking Agent Runtime follow-up; source-architecture `L0` remains admitted |
...
| Cross-consumer | External product claims lacked executable tree/blob/wiring evidence | Added exact Git/AST source records plus restricted Frontend contribution and Agent Runtime capability-root checks; runtime behavior and independence remain unproved |
...
| Governance | Accepted ADR-0013 and ADR-0014 assign overlapping authoring authority | A successor decision is a mandatory fail-closed prerequisite for a new authoring rehearsal |
