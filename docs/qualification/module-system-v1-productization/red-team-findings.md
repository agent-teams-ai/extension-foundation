...
| Product | Frontend was promoted to Static V1 GO from self-asserted file records | Downgraded it to a no-go measurement candidate and added exact Git custody plus literal named-topology verification |
| Product | Agent Runtime was recorded before its Codex and Claude Code production paths landed | The historical `493c6c3` lineage was superseded by the exact current source record at `7be9982`; recorded `GO_PRODUCT_SOURCE_TOPOLOGY` while keeping runtime behavior and `L1-L5` closed |
| Product | Codex and Claude Code could be mistaken for interchangeable providers | Named-call topology records their declared capability/member and host dependency property names without claiming value linkage or runtime behavior |
| Testing | Claude Code lacks the direct default-root test shape already present for Codex | Recorded a non-blocking Agent Runtime follow-up; exact-source named-call topology remains the only `L0` verdict |
...
| Cross-consumer | External product claims lacked executable tree/blob/topology evidence | Added exact Git/exported-symbol records plus narrow Frontend literal-provider and Agent Runtime named-call topology checks; semantic dataflow, runtime behavior, and independence remain unproved |
